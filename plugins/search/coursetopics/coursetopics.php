...
class plgSearchCoursetopics extends JPlugin
{
    ...
    */
    function plgSearchCoursetopics( $text, $phrase='', $ordering='', $areas=null )
    {
        ...
        $rows = JoomdleHelperContent::call_method ("search_topics", $text,$phrase,$ordering,  (string) $limit, $langtag);
        ...
        foreach($rows as $key => $row) {
			...
			$rows_result[$i]->href = $this->get_section_link ($row);
    ...
    private function get_section_link ($row)
    {
        $linkstarget = $this->params->def('linkstarget', 'topics');
        ...
        switch ($linkstarget)
        {
            case 'moodle':
                ...
                $link = $jump_url."&mtype=course&id=".$row['remoteid']."&Itemid=$itemid"."&section=2";
            ...
            default:
                ...
                $link = 'index.php?option=com_joomdle&view=topics&cat_id='.$cat_slug.'&course_id='.$course_slug;
                break;
        }
