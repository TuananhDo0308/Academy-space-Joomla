...
class JoomdleController extends JControllerLegacy {
    ...
    function register_save ()
    {
        ...
        if (($useractivation == 1) || ($useractivation == 2))
        {
                ...
                $user->set('activation', JUserHelper::hashPassword (JUserHelper::genRandomPassword()));
                $user->set('block', '1');
        }
    ...
    {
        $app = JFactory::getApplication();
        $app->redirect (JURI::root () . 'index.php?option=com_joomdle&task=getout_logout');
    }
    ...
    function getout_login ()
    {
        $root = JURI::root ().'index.php?option=com_users&view=login';
        ?>
		<script type="text/javascript">
    ...
    function getout_logout ()
    {
        $root = JURI::root ().'index.php?option=com_joomdle&task=do_logout';
        ?>
		<script type="text/javascript">
		top.window.location = "<?php echo $root; ?>";
		</script>
